...
# Use the installer pinned by the vendor submodule. Copy it before rewriting
# registry and release URLs so the upstream checkout stays unchanged.
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
VENDOR_INSTALL_SCRIPT="$SCRIPT_DIR/vendor/install.sh"
...
cp "$VENDOR_INSTALL_SCRIPT" "$INSTALL_SCRIPT"
...
rewrite_install_var() {
    ...
    if ! sed -n "s|^$var=||p" "$INSTALL_SCRIPT" | grep -qxF "$value"; then
        echo "Error: could not rewrite $var inside the vendored pnpm install script." >&2
        exit 1
    fi
}
...
rewrite_download_base() {
    ...
    if ! grep -qF "$GITHUB_RELEASES_BASE" "$INSTALL_SCRIPT"; then
        echo "Error: could not find '$GITHUB_RELEASES_BASE' inside the vendored pnpm install script." >&2
        exit 1
    fi
    sed -i "s|$GITHUB_RELEASES_BASE|$value|g" "$INSTALL_SCRIPT"
    if ! grep -qF "$value" "$INSTALL_SCRIPT"; then
        echo "Error: could not rewrite '$GITHUB_RELEASES_BASE' inside the vendored pnpm install script." >&2
        exit 1
    fi
}
